...
def local_mechanism(s:DiscoveryState,T_C:float,p:DiscoveryParams):
    ...
    available=max(top['C_TJ']+.05,0)*max(top['f_clean_GB'],.02)
    ...
    mobility_multiplier=gamma/(1+Rpersistent);Gdot=oldG*mobility_multiplier
    ...
    return {**d,'rho_dot':den,'G_dot':Gdot,'X_J':s.X_J,'X_J_dot':Xdot,'X_J_production':production,'X_J_relaxation':relaxation,'R_J_persistent':Rpersistent,'Lambda_TJ':Lambda,'K_TJ':K,'Lambda_over_K_TJ':Lambda/max(K,1e-30),'P_comp_TJ':gamma,'TJ_regime':regime,'growth_mobility_discovery':d['growth_mobility_factor']*mobility_multiplier,'P_persistent_junction_drag':preg*(Rpersistent/max(Rpersistent+(1-gamma),1e-30)) if persistent else 0.,'P_TJ_multihit':preg*((1-gamma)/max(Rpersistent+(1-gamma),1e-30)) if multihit else 0.,'P_clean_GB_discovery':migration_part*mobility_multiplier}
