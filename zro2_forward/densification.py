...
def kinetic_state(sigma_Pa: float, T_K: float, G_m: float, connectivity: float,
                  barrier, material, C1: float = 1.0) -> dict[str, float]:
    ...
    edot = np.clip(connectivity, 0, 1)*geo["eps_event"]*activity/max(tau, 1e-300)
    return {"Gstar_J": Gstar, "r_nuc_sinv": rate, "tau_sink_s": tau,
            "Lambda": Lambda, "activity": activity, "edot_sinv": edot, **geo}
